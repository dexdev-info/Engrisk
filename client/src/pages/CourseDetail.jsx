...
const CourseDetail = () => {
    ...
    if (loading) return <div className="flex justify-center items-center h-[50vh]"><Spin size="large" /></div>;
    if (!course) return <div className="text-center mt-20">Khóa học không tồn tại</div>;

    // Check user enrollment locally (Simple check)
    const isEnrolled = user?.enrolledCourses?.includes(course._id) || course.isEnrolled;
        ...
        <div className="max-w-6xl mx-auto p-4 md:p-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2">
                    <Title level={1} className="!mb-4">{course.title}</Title>
                    <div className="flex items-center gap-4 mb-6">
                        <Tag color="blue" className="px-3 py-1 text-sm">{course.level}</Tag>
                    ...
                    <Title level={3} className="!mb-4">Nội dung khóa học</Title>
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        <List
                            ...
                            renderItem={(lesson, index) => (
                                <List.Item
                                    className={`px-6 py-4 hover:bg-gray-50 transition-colors cursor-pointer border-b last:border-b-0 ${isEnrolled ? 'hover:pl-8' : 'opacity-70'
                                        }`}
                                    onClick={() => {
                                        if (isEnrolled) {
                                ...
                                >
                                    <List.Item.Meta
                                        avatar={
                                            <Avatar
                                                shape="square"
                                                size="large"
                                                className={isEnrolled ? "bg-blue-100 text-blue-600" : "bg-gray-100 text-gray-400"}
                                                icon={isEnrolled ? <PlayCircleOutlined /> : <LockOutlined />}
                                            />
                                        }
                                        title={<Text strong className="text-base">Bài {index + 1}: {lesson.title}</Text>}
                                        description={
                                    ...
                                    />
                                    {/* Nút Action bên phải */}
                                    {isEnrolled && <Button size="small" type="primary" ghost>Học ngay</Button>}
                                </List.Item>
                            )}
                        />
                    </div>
                ...
                <div className="lg:col-span-1">
                    <div className="sticky top-24">
                        <Card
                            cover={<img alt={course.title} src={course.thumbnail || 'https://via.placeholder.com/400x250'} className="h-48 object-cover" />}
                            className="shadow-lg border-0 overflow-hidden rounded-xl"
                        >
                            <div className="text-center">
                                <Title level={2} className="!mb-2 !text-blue-600">Miễn phí</Title>
                                <Text type="secondary" className="line-through mr-2">1.200.000đ</Text>
                                <Tag color="red">FREE</Tag>

                                <Divider className="my-4" />

                                {isEnrolled ? (
                                    <Button type="primary" size="large" block className="h-12 text-lg font-bold bg-green-600 hover:bg-green-500 border-none">
                                        TIẾP TỤC HỌC
                                    </Button>
                                ) : (
                                    <Button
                                ...
                                <div className="mt-4 space-y-2 text-left text-gray-600 text-sm">
                                    <p className="flex items-center gap-2"><CheckCircleOutlined className="text-green-500" /> Truy cập trọn đời</p>
                                    <p className="flex items-center gap-2"><CheckCircleOutlined className="text-green-500" /> Học trên mọi thiết bị</p>
                                    <p className="flex items-center gap-2"><CheckCircleOutlined className="text-green-500" /> Cấp chứng chỉ khi hoàn thành</p>
                                </div>
                            </div>
                        </Card>
                    </div>

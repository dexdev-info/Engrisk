...
export const useVocabList = ({
  ...
  const fetchVocabs = useCallback(async () => {
    ...
    try {
      ...
      const res = await vocabService.getAll({
  ...
  // Hàm update nhanh UI mà không cần fetch lại
  const updateVocabLocal = useCallback((vocabId, changes) => {
    setVocabs((prevVocabs) =>
      prevVocabs.map((v) => (v._id === vocabId ? { ...v, ...changes } : v))
    )
  }, [])
